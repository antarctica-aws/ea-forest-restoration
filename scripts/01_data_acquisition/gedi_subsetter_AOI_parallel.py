...
#             sys.exit(2)
    ...
    #     # Check for empty output dataframe
        try:    
            ...
            with open(".../EA_data/l2b_orig/all_ea/gediFiles_l2b.txt", "r") as fp:
                lines = fp.readlines()

            with open(".../EA_data/l2b_orig/all_ea/gediFiles_l2b.txt", "w") as fp:
                for line in lines:
                    if line.strip("\n") != g:
                        fp.write(line)
...
ROI = '.../data/forest_cube.geojson'   ###[change]####
outpath = '.../data/l2b_subset/forest_cube/'    ###[change]####
gediFiles =  glob(path.join('.../data/l2b_orig/forest_sube', 'GEDI02_B*.h5')) #data.split("\n")   ###[change]####
# print(gediFiles)
# my_file.close()
t=len(gediFiles)
print(t)
